...
- **§6.2's overlap question is asked at spawn**, since a shared worker never reaches the merge that used to ask it — and it matters more here: two isolated workers on one file get a conflict the gate catches, while two shared workers get a last-write-wins race with nothing watching. `worker_spawn` warns when a worker claims paths another live worker already claims (naming it, and the three ways out) and when a shared worker declares no `ownedPaths` at all, which leaves it unbounded and its work unattributable.
